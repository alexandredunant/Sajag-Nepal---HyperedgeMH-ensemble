...
Robinson et al. \citep{Robinson2018} emphasized shaking-induced impacts concentrated near rupture zones, providing valuable insights through scenario ensemble approaches. Building on this previous work, our study incorporates landslides as cascading hazards to expand the analysis. Overall, the Spearman correlation of 0.79 with the previous work confirms that the order of impact at district level are highliy correlated (Figure \ref{fig:S5}). This is expected as the earthquake ensemble members are similar and represent a large part of the multi-hazard damage, as shown in the Result section. The multi-hazard scenarios reveal broader and more specific spatial impacts beyond zones affected by shaking, especially in steep, landslide-prone areas (Figure \ref{fig:worst_case_maps} and \ref{fig:standard_deviation}). Figure~\ref{fig:district_exceedance} shows that districts in the High Mountain and Hill physiographic regions experience higher exceedance probabilities of landslide impacts than shaking impacts alone.

Substantial variation was also observed in the proportion of damage caused by shaking versus landslides (Figure~\ref{fig:district_ratio}). Some districts that appeared moderately affected in shaking-only scenarios showed higher overall impacts when landslides were included. These variations point to the need for localized risk management strategies that consider both seismic and landslide hazards.
...
The relative importance of secondary hazards varies substantially with spatial scale and underlying exposure patterns. In densely populated districts with large building stocks, landslide impacts can represent a modest addition to shaking damage—typically increasing total impacts by 10--20\%. However, in smaller, mountainous districts with steep terrain and limited infrastructure, landslides can effectively come close to double the total hazard burden relative to shaking alone (Figure~\ref{fig:district_ratio}). While landslides can constituted a small fraction of national-level damage, they fundamentally altered the risk profile in this specific locality, transforming a potentially moderate shaking scenario into a catastrophic multi-hazard event. Planning frameworks that rely solely on national or regional aggregate statistics may therefore systematically underestimate risk in vulnerable localities where cascading hazards dominate.
