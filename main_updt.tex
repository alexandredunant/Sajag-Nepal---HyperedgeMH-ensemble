...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\linewidth]{./FIGS/Fig1_study_area_map.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\linewidth]{./FIGS/Fig2_exceedance_probability.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\linewidth]{./FIGS/Fig3_district_impacts_ratio.png}
...
District-level variability is further explored in Figure~\ref{fig:worst_case_maps} (Panels C, F, and I), which maps the standard deviation of impacts for each district. The standard deviation is generally lower in the Terai districts and higher elsewhere in Nepal, although there are no clear spatial trends. Standard deviation is also somewhat higher for landslide-related impacts than for shaking-induced building damage. This indicates that while shaking damage was relatively consistent across scenarios, landslide-related impacts were more variable and scenario-dependent, with significant impacts occurring only under certain scenarios.

Importantly, building damage from shaking and building or road damage from landsliding have different spatial footprints across Nepal. Districts with the largest expected numbers of damaged buildings, both in the median and in the worst-case scenarios, are concentrated in the more populated southern and central areas of the country (Figure~\ref{fig:worst_case_maps}A), highlighting not only building density but also patterns of infrastructure vulnerability. Districts in the more remote and more sparsely-populated northeast have relatively low total numbers of shaking-damaged buildings, even in the worst case. In contrast, while the absolute numbers of expected landslide impacts are lower, the largest number of landslide impacts are concentrated in the hill and mountain districts across Nepal (Figure~\ref{fig:worst_case_maps}B, C). Road impacts from landslides are widely distributed through the hill and mountain districts, with particular hotspots in Sindhupalchok, Baglung, Rolpa, Bajura, and Bajhang districts (Figure~\ref{fig:worst_case_maps}B). These areas should be considered at the highest risk of disruption to the road network after a future large earthquake. Building impacts from landslides are more focused on the hill districts, likely because of the concentration of population in those areas (Figure~\ref{fig:worst_case_maps}C). Not surprisingly, expected landslide impacts are lower in the flat-lying southern districts of the Terai, along the border with India (Figure~\ref{fig:worst_case_maps}C, E). The dominance of shaking impacts over landslide impacts means that their sum, which we take as the worst-case expected total number of earthquake impacts, mirrors the distribution of shaking damage, although numbers of impacts are notably higher in the hill districts in particular (compare Figure~\ref{fig:worst_case_maps}D and 6A).

\begin{figure*}
    \centering
    \includegraphics[width=1\textwidth]{./FIGS/Fig5_worst_case_absolute_proportional.png}
    \caption{Maps of earthquake impacts by district. Left column: Worst-case absolute numbers of (A) buildings damaged by shaking, (D) buildings damaged by landslides, and (G) road segments damaged by landslides. Right column: Worst-case proportion (in percent) of (B) buildings damaged by shaking, (E) buildings damaged by landslides, and (H) road segments damaged by landslides.}
    \label{fig:worst_case_maps}
\end{figure*}

\begin{figure}
    \centering
    \includegraphics[width=0.75\linewidth]{FIGS/Fig7_standard_deviation.png}
    \caption{Standard deviation of (C) buildings damaged by shaking, (F) buildings damaged by landslides, and (I) road segments damaged by landslides. Note the different scales between panels.}
    \label{fig:placeholder}
\end{figure}

\subsection{Vulnerability and accessibility}

The index of remoteness from district headquarters (Figure~\ref{fig:remoteness}A) shows values ranging from near 0 in southern districts to over 0.8 in northern regions, with the highest values observed in the northwestern districts of Dolpa, Bajhang, Darchula, Humla and Bajura. The remoteness index from health facilities (Figure~\ref{fig:remoteness}B) exhibits a similar geographical pattern but with generally lower values throughout the country. The scatter plot comparing these two indices (Figure~\ref{fig:remoteness}C) shows most districts positioned below the 1:1 line, with remoteness values from district headquarters typically higher than those from health posts for the same districts. Identified outliers include Bajhang and Darchula with the district of Manang being the outlier above the 1:1 line. The boxplot analysis (Figure~\ref{fig:remoteness}D) illustrates that the percentage of population with access to facilities within one hour is higher for health posts (median ~5.1\%) than for district headquarters (median ~1.8\%).

\subsection{Remoteness, impacts, and risk scores}

The relationship between remoteness and anticipated road damage by landsliding is presented in Figure~\ref{fig:remoteness} (bottom panels). There is a weak but positive correlation between district-averaged remoteness index and percentage of road segments damaged by landslides. This correlation becomes even more pronounced when we divide districts between low, medium, and high remoteness scores; median values of the worst-case percentage of road segments damaged by landslides increase with increasing remoteness. The combined risk score incorporating both road damage and remoteness shows that the highest values (yellow) are concentrated in the north-western regions of the country.

Figure~\ref{fig:remoteness} also depicts relationships between road vulnerability and healthcare accessibility. The data points follow a nonlinear pattern with access rates declining more steeply at higher road risk scores. The combined road intervention prioritization matrix divides districts into four quadrants by median values for remoteness (y-axis) and landslide vulnerability (x-axis). The upper-right quadrant (high remoteness, high road damage) indicates highest priority for interventions. Districts within this quadrant include Dolpa, Bajhang, and Darchula, with remoteness indices above 0.8 and road damage percentages exceeding 20\%.

\begin{figure}[H]
    \centering
    \includegraphics[width=1\linewidth]{./FIGS/Fig10_remoteness_analysis.png}
    \caption{Combined analysis of facility-specific remoteness and road vulnerability in Nepal. Top panels (Facility Remoteness): A, remoteness from district headquarters during the dry season. B, remoteness from health facilities during the dry season. C, comparison of facility-specific remoteness indices showing correlation between different facility types (dashed line shows 1:1). D, proportion of the population with less than 1 hour travel time to essential services, by facility type. Bottom panels (Road Vulnerability & Intervention): (Left) Relationship between remoteness and road damage vulnerability; (Middle) Road damage impacts by remoteness category and combined road damage and remoteness risk score; (Right) Road Intervention Prioritization Matrix organizing districts by both remoteness and landslide vulnerability. High-priority districts in the upper-right quadrant include Dolpa, Bajhang, and Darchula, all with remoteness indices above 0.8 and road damage percentages exceeding 20\%.}
    \label{fig:remoteness}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=1\linewidth]{./FIGS/Fig11_risk_scores.png}
...
\begin{figure}[H]
    \centering
    \fbox{\parbox[c][6cm][c]{0.8\textwidth}{Placeholder for Figure S1}}
...
\begin{figure}[H]
    \centering
    \fbox{\parbox[c][6cm][c]{0.8\textwidth}{Placeholder for Figure S2}}
...
\begin{figure}[H]
    \centering
    \fbox{\parbox[c][6cm][c]{0.8\textwidth}{Placeholder for Figure S3}}
...
\begin{figure}[H]
    \centering
    \fbox{\parbox[c][6cm][c]{0.8\textwidth}{Placeholder for Figure S4}}
