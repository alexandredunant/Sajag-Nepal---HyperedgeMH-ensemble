...
where $R$ is the normalized remoteness index, $S$ is the absolute worst-case shaking damage (number of buildings), and $L$ is the absolute worst-case landslide damage (number of buildings and road segments combined). The total impact $(S + L)$ is normalized before being combined with remoteness in a weighted sum. This approach treats remoteness and disaster magnitude as two independent but equally important components (0.5 weight each). Districts score high if they have either high remoteness or high total impact, or both. This score helps identify areas requiring substantial humanitarian response.
...
\begin{figure}[H]
    ...
    \caption{Standard deviation of (A) buildings damaged by shaking, (B) buildings damaged by landslides, and (C) road segments damaged by landslides.}
...
The relationship between remoteness and road network vulnerability is further detailed in Figure~\ref{fig:remoteness} bottom panels. Figure~\ref{fig:remoteness}C demonstrates that districts categorized as having 'High' remoteness experience a significantly larger proportion of road network damage (median $\approx 24\%$) compared to 'Low' remoteness districts (median $\approx 9\%$). Conversely, Figure~\ref{fig:remoteness}D shows that in terms of absolute numbers, the 'Low' remoteness districts (often more urbanized) bear the highest total impact count (shaking and landslides combined). Based on the combination of hazard and remoteness \eqref{eq:remoteness-index} that has been computed and the equation \eqref{eq:absolute-risk-score} and \eqref{eq:normalized-risk-score}, we create the risk score maps presented Figure  \ref{fig:risk_score}. Figure \ref{fig:risk_score} visualizes the spatial heterogeneity of risk by contrasting gross exposure with localized intensity. While Panel A maps the aggregate score to identify regions facing the highest total burden of compounding disasters, Panel B applies a standardization technique to account for local vulnerabilities (i.e. remotness), thereby exposing communities subjected to the most acute risk, regardless of their absolute numbers.
...
Figure \ref{fig:risk_score} illustrates how different risk metrics reveal distinct spatial patterns of earthquake vulnerability across Nepal. Panel A presents the Absolute Compounding Risk Score, which combines remoteness with total worst-case damage (buildings and roads). This metric identifies districts facing the largest aggregate disaster burden, with elevated scores scattered around Nepal, in districts such as Bajhang, Dolpa, Chitawan, Sindhupalchok, Dang and Morang. These areas score highly due to either substantial absolute infrastructure exposure, high remoteness, or both factors combined. Panel B shows the Normalized Risk Score, which accounts for proportional damage relative to each district's infrastructure and population size. This normalization fundamentally shifts the risk geography, revealing acute vulnerabilities in northwestern districts where communities face disproportionately high percentages of infrastructure loss in combination with high remoteness. 

\section{Discussion}

\subsection{Differences between shaking-only and multi-hazard scenario outcomes}

The strong Spearman rank correlation of 0.79 between the district-level impact rankings derived here and those of \citet{Robinson2018} confirms that incorporating landslides as a cascading hazard does not fundamentally reorder the relative risk profile of Nepal's districts at the national scale (Figure~\ref{fig:S5}). This result is expected: the two analyses share the same 30-member earthquake ensemble, and shaking damage dominates total impact counts in the large majority of districts. However, the correlation conceals meaningful divergence in specific districts -- particularly those in the High Mountain and Hill physiographic regions -- where landslide impacts can constitute up to 84\% of the number of buildings damaged by shaking (Figure~\ref{fig:district_ratio}). These districts represent the principal added value of the multi-hazard approach: under a shaking-only assessment, many of them appear only moderately affected, yet the inclusion of earthquake-triggered landslides substantially alters their risk profile. This pattern is consistent with the documented tendency of shaking-only frameworks to underestimate risk in areas of high topographic relief where secondary hazards are prevalent \citep{Budimir2014}, and reinforces the need for multi-hazard approaches in continental earthquake settings.

The spatial variability in the ratio of landslide to shaking damage (Figure~\ref{fig:district_ratio}) further highlights the role of local topography and landslide susceptibility in shaping district-level impact patterns. Districts in the low-relief Terai show consistently low landslide contributions, while northern mountain districts such as Darchula, Manang, and Dolpa show the highest ratios. This heterogeneity implies that risk thresholds derived from national or regional aggregate statistics will systematically misrepresent the relative severity of risk in topographically complex settings, and that locally-informed, multi-hazard assessments are necessary to support effective contingency planning across Nepal's diverse physiographic regions.

\subsection{Road network impacts and compounding effects of remoteness}

The quantification of landslide damage to the road network represents one of the most operationally significant contributions of this study relative to previous scenario ensemble approaches for Nepal. Road infrastructure is the primary means of access for emergency responders, medical supplies, and humanitarian personnel in Nepal's hilly and mountainous districts, and its disruption following a large earthquake can substantially delay response times and compound the human consequences of the event \citep{Koks2019, NationalPlanningCommission2015}. Our results demonstrate that the proportion of the road network damaged by landslides is substantially higher in districts classified as highly remote (median $\approx 24\%$) than in those with low remoteness (median $\approx 9\%$), and this disparity is compounded by the fact that remote districts already face longer pre-event travel times to health facilities and district headquarters (Figure~\ref{fig:remoteness}). The effect is self-reinforcing: the physical remoteness that elevates landslide exposure is simultaneously the characteristic that makes post-event access most critical and most difficult to restore. Districts such as Dolpa, Bajhang, and Darchula illustrate this compound vulnerability most acutely, combining high remoteness indices with consistently elevated road damage probabilities across the scenario ensemble.

These findings have direct implications for pre-event contingency planning. The identification of districts where road damage is both severe and consistent across multiple scenarios provides a basis for prioritising investments in alternative access options, including pre-positioning of emergency supplies, the development of secondary route alternatives, and planning for aerial transport capacity in the most inaccessible areas. In districts where road disruption is a near-certain consequence of a large earthquake, post-event logistics capacity cannot be assumed, and pre-event preparation is correspondingly critical \citep{IASC2007}.

\subsection{Scale dependence of hazards}

The relative importance of secondary hazards varies substantially with spatial scale and underlying exposure patterns. In densely populated districts with large building stocks and low topographic relief, landslide impacts represent a small addition to shaking damage, sitting at the lower end of the 1--84\% range observed across all districts (Figure~\ref{fig:district_ratio}). However, in smaller, mountainous districts with steep terrain and limited infrastructure, landslides can constitute up to 84\% of the total shaking damage burden (Figure~\ref{fig:district_ratio}). While landslides account for a small fraction of national-level damage -- with totals 1--2 orders of magnitude below shaking impacts (Figure~\ref{fig:exceedance}) -- they fundamentally alter the risk profile in these specific localities, transforming what would otherwise appear as a moderate shaking scenario into a potentially catastrophic multi-hazard event. Planning frameworks that rely solely on national or regional aggregate statistics may therefore systematically underestimate risk in vulnerable localities where cascading hazards dominate.

\subsection{Urban--rural risk contrast and geographic disparities}

The results reveal a spatially structured urban--rural risk divide with direct implications for disaster risk governance in Nepal. Urban and peri-urban districts in the Terai and major river valleys face the greatest absolute burden from earthquake shaking, a consequence of their high population density and building stock. However, the most severe proportional losses -- and the greatest challenges for post-event response -- are concentrated in remote rural districts of the Hill and Mountain zones. These districts face disproportionately high percentages of road network damage in combination with high remoteness from health facilities and government support (Figures~\ref{fig:worst_case_maps} and \ref{fig:remoteness}). The communities least equipped to absorb and recover from disaster impacts are therefore the same communities whose physical connectivity is most vulnerable to earthquake-triggered landslides. This pattern reflects longstanding disparities in infrastructure investment and institutional capacity across Nepal's physiographic regions \citep{NationalPlanningCommission2014}, which compound the physical risk differentials identified here. The persistence of such geographic inequalities in disaster risk and response capacity has been widely documented across multi-hazard settings \citep{Wisner2004}.

\subsection{Limitations}

Several limitations are inherent to the ensemble approach. While the analysis captures large-scale impacts from high-magnitude earthquakes ($M_w > 7$), it does not include locally destructive moderate-magnitude events such as the 2023 Jajarkot earthquake ($M_w$ 5.7). These events can cause severe localised impacts and warrant dedicated modelling approaches. Additionally, we focus here on physical infrastructure damage and do not explicitly model fatalities, as infrastructure impact alone is not sufficient for risk-to-life estimates from landslides \citep{VanWykDeVries2024}. Future work integrating dynamic exposure and population mobility data is expected to improve casualty estimation for both shaking and landslide hazards.

A further limitation concerns the transferability of the susceptibility and fragility models across the ensemble. Both the landslide susceptibility model and the building fragility functions were calibrated using observations from the 2015 $M_w$ 7.8 Gorkha earthquake, which involved thrust rupture on the Main Himalayan Thrust beneath central Nepal. Several of the 30 scenario earthquakes considered here involve different fault systems and shaking geometries -- including the Karakoram Fault and the Thakkhola Graben -- for which no independent calibration data are currently available. The degree to which the susceptibility and fragility relationships derived from the Gorkha event generalise to these different tectonic and geomorphic settings is uncertain and represents an important topic for future investigation.

Finally, the calculation of expected district-level impacts assumes statistical independence of damage between individual buildings and road segments. In practice, co-location of multiple assets within large landslide source or runout areas could introduce positive spatial correlation in outcomes, in which case our approach may underestimate the variance of impacts in districts with large, contiguous slope units. Future work incorporating spatially correlated failure models would help to quantify this additional source of uncertainty.
