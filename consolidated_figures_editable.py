...
# The conda env's PROJ_DATA points to an older proj.db that rasterio's PROJ rejects,
# which makes every contextily basemap fail silently. Use rasterio's bundled PROJ data.
try:
    import importlib.util
    _rio_spec = importlib.util.find_spec("rasterio")
    _rio_proj = os.path.join(os.path.dirname(_rio_spec.origin), "proj_data") if _rio_spec else None
    if _rio_proj and os.path.exists(os.path.join(_rio_proj, "proj.db")):
...
output_dir = os.path.join("/mnt/CEPH_PROJECTS/Proslide/Alex/SajagN/Sajag-Nepal---HyperedgeMH-ensemble/FIGURES/formats")
...
# District boundaries in grey (per reviewer request) so the black fault traces stand out
districts_mercator.plot(ax=ax1, facecolor='orange', edgecolor='gray', linewidth=2.0, alpha=0.4, zorder=2)
...
# Slightly more transparent background (per reviewer request) to improve fault visibility
try:
...
if os.path.exists(ward_file):
    try:
        ...
        wards_mercator.plot(ax=ax1, facecolor='none', edgecolor='gray', linewidth=0.15, alpha=0.5, zorder=3)
        print("Ward boundaries plotted")
    except Exception as e:
        print(f"Could not plot ward boundaries: {e}")

# Load and plot faults with thickened lines
fault_file = os.path.join(data_dir, "shp", "faults", "Nepal_Faults_DMG.shp")
fault_gmt_file = os.path.join(base_dir, "ARCHIVE", "figs", "GMT", "HimaTibetMap.geojson")

if os.path.exists(fault_file):
    try:
        faults = gpd.read_file(fault_file)
        faults_mercator = faults.to_crs("EPSG:3857")
        # Faults plotted in red so they are distinct from the black outlines of the
        # districts discussed in the text and stand out against the basemap;
        # legend label updated to "Department of Mines and Geology, Govt of Nepal"
        faults_mercator.plot(ax=ax1, color='#D7191C', linewidth=4.0, alpha=0.9, zorder=4, label='Department of Mines and Geology, Govt of Nepal')
        print("DMG fault lines plotted (thickened, red)")
    except Exception as e:
        print(f"Could not plot DMG faults: {e}")
else:
    print(f"WARNING: fault file not found, faults NOT plotted: {fault_file}")
...
# Districts explicitly discussed by name in the main text (bolded per reviewer request).
# Matched against the shapefile's 'DISTRICT' field.
...
# bolded districts are drawn last with the highest zorder and an opaque box so that
# neighbouring labels and fault traces never cover them.
matched = set(districts_mercator['DISTRICT'].str.title()) & {d.title() for d in highlighted_districts}
missing = {d.title() for d in highlighted_districts} - matched
if missing:
    print(f"WARNING: highlighted districts not found in shapefile: {sorted(missing)}")
highlight_mask = districts_mercator['DISTRICT'].str.title().isin(matched)
# Outline the discussed (bolded) districts in black so they stand out from the grey boundaries
districts_mercator[highlight_mask].plot(ax=ax1, facecolor='none', edgecolor='black',
                                        linewidth=2.5, zorder=3.5)
for is_highlighted in (False, True):
    for idx, row in districts_mercator[highlight_mask == is_highlighted].iterrows():
        centroid = row.geometry.centroid
        ax1.annotate(row['DISTRICT'].title(), xy=(centroid.x, centroid.y),
                    fontsize=14, fontweight=('bold' if is_highlighted else 'normal'), color='black',
                    ha='center', va='center', zorder=(7 if is_highlighted else 5),
                    bbox=dict(boxstyle='round,pad=0.2', facecolor='white',
                              alpha=(0.95 if is_highlighted else 0.7), edgecolor='none'))

print(f"Added labels for all {len(districts)} districts ({len(highlighted_districts)} bolded as discussed in text)")
...
# Set aspect ratio ('box' keeps the data limits, so the basemap tiles fill the frame)
...
fig1.savefig(os.path.join(output_dir, "Fig1_study_area_map.pdf"), bbox_inches='tight')
fig1.savefig(os.path.join(output_dir, "Fig1_study_area_map.eps"), bbox_inches='tight', format='eps')
...
fig2.savefig(os.path.join(output_dir, "Fig2_exceedance_probability.eps"), bbox_inches='tight', format='eps')
...
# Always plot in Web Mercator so axis limits are always correct
...
# Optionally overlay basemap (skip silently if PROJ unavailable)
try:
    import contextily as ctx
    ...
    print(f"Could not add basemap (map still correct): {e}")
    ax3.set_facecolor('lightgray')

def _fmt_lon_fig3(x, pos):
    lon = x / 20037508.34 * 180
    return f'{lon:.1f}E'

def _fmt_lat_fig3(y, pos):
    lat = (2 * np.arctan(np.exp(y / 20037508.34 * 180 * np.pi / 180)) - np.pi / 2) * 180 / np.pi
    return f'{lat:.1f}N'

ax3.xaxis.set_major_formatter(ticker.FuncFormatter(_fmt_lon_fig3))
ax3.yaxis.set_major_formatter(ticker.FuncFormatter(_fmt_lat_fig3))
...
fig3.savefig(os.path.join(output_dir, "Fig3_district_impacts_ratio.eps"), bbox_inches='tight', format='eps')
...
fig4.savefig(os.path.join(output_dir, "Fig4_district_exceedance_curves.eps"), bbox_inches='tight', format='eps')
plt.show()
...
fig5.savefig(os.path.join(output_dir, "Fig5_worst_case_absolute_proportional.eps"), bbox_inches='tight', format='eps')
...
fig6.savefig(os.path.join(output_dir, "Fig6_standard_deviation.pdf"), bbox_inches='tight')
fig6.savefig(os.path.join(output_dir, "Fig6_standard_deviation.eps"), bbox_inches='tight', format='eps')
...
fig9.savefig(os.path.join(output_dir, "Fig7_remoteness_analysis.pdf"), bbox_inches='tight')
fig9.savefig(os.path.join(output_dir, "Fig7_remoteness_analysis.eps"), bbox_inches='tight', format='eps')
...
fig10.savefig(os.path.join(output_dir, "Fig8_risk_scores.pdf"), bbox_inches='tight')
fig10.savefig(os.path.join(output_dir, "Fig8_risk_scores.eps"), bbox_inches='tight', format='eps')
...
robinson_file = os.path.join(data_dir, "Robinson2018", "robinson_district_scores.csv")
